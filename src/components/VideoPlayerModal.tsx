...
export const VideoPlayerModal: React.FC<VideoPlayerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-stone-950/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 animate-fadeIn">
      {/* Modal Container */}
      <div className="bg-white border border-rose-200 shadow-2xl flex flex-col overflow-hidden transition-all duration-300 rounded-2xl w-full max-w-[580px] max-h-[92vh]">
        {/* Header */}
        <div className="px-4 py-3 bg-rose-50/60 border-b border-rose-100 flex items-center justify-between gap-3 shrink-0">
          ...
          <div className="flex items-center gap-1 shrink-0">
            <button
              ...
              className={`hidden p-1.5 border transition-colors rounded-lg cursor-pointer ${
                video.isWatchLater
                  ...
                  : 'border-rose-200 bg-white text-stone-600 hover:text-rose-700'
              }`}
              title="Queue"
            >
            ...
            <button
              ...
              className={`hidden p-1.5 border transition-colors rounded-lg cursor-pointer ${
                video.isFavorite
                  ...
                  : 'border-rose-200 bg-white text-stone-600 hover:text-rose-600'
              }`}
              title="Starred"
            >
            ...
            <button
              onClick={handleCopy}
              className="hidden p-1.5 border border-rose-200 bg-white text-stone-600 hover:text-stone-900 transition-colors rounded-lg cursor-pointer"
              title="Copy Link"
            >
              {copied ? <Check className="w-3.5 h-3.5 text-rose-600" /> : <Share2 className="w-3.5 h-3.5" />}
            </button>
            <a
              href={video.url}
              target="_blank"
            ...
            <button
              onClick={onClose}
              className="p-1.5 text-stone-400 hover:text-stone-800 hover:bg-rose-100/70 transition-colors rounded-lg ml-0.5 cursor-pointer"
              title="Close"
            >
              <X className="w-4 h-4" />
            </button>
        ...
        <div className="flex-1 overflow-y-auto flex flex-col min-h-0 bg-white">
          ...
          {/* Guide & Meta */}
          <div className="p-4 flex items-center justify-between text-xs border-t border-rose-100">
